...
@app.cell
def _(df_errors):
    df_errors.groupby("experiment").median()
    return
...
@app.cell
def _(df_errors, np, results):
    true_time = np.array(results["scritmo_context_model"]["ext_time_hours"])
    n_experiments = len(results)
    df_errors["true_time"] = np.tile(true_time, n_experiments)
    return
