...
def _(mo):
    mo.md(r"""
    # Phase 5 Results — JEPA vs scritmo baseline

    Comparison of circadian phase inference on synthetic realistic (NB-generated) data.
    **1 500 cells · 15 clock genes · 6 discrete timepoints · 250 cells each.**
    """)
    return
...
def _(df_errors, order_map, plt, sns):
    ...
    _ax.set_title("Per-cell phase error — JEPA variants vs scritmo baseline", fontsize=11)
...
def _(np, plt, results, shorten_vm, sns):
    ...
    _ax.set_xticklabels(_labels_s, fontsize=8)
...
def _(TWO_PI, align_phase_local, np, plt, results, shorten_vm):
    ...
    fig_polar.suptitle(
        "Polar histograms of predicted phases per true timepoint",
        fontsize=12, y=1.02,
    )
    plt.tight_layout()
    fig_polar
